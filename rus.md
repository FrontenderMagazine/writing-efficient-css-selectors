...
дизайнера интерьеров. Её стоит знать в любом случае.
...
Менее распространённые селекторы включают базовые псевдоклассы (такие как `:hover`),
...
Цитата из книги «Как сделать сайты ещё быстрее» (Even Faster Websites) [Стива Саудерса][1]

> Важно отметить, что хотя селекторы по ID технически быстрее, с точки зрения
производительности, разница очень невелика. Вы можете сами в этом убедиться с
...
Однако, тест селектора по типу (`<a>`), в отличии от селекторов по классу или ID,
выдал [гораздо большее время перерисовки страницы][4].

Тест на серьёзно перенасыщенном селекторе с вложенными элементами дал [около 440 мс][5]!

Становится очевидным, что разница между селекторами по ID/классам и типам/вложенным элементам
...
подниматься вверх по дереву DOM, чем начинать _сверху_ и потом спускаться по дереву
...
производительным селектором. Браузер найдёт все элементы .intro (которых, скорее
...
Однако, именно такой селектор не силён в плане производительности:
...
Предположим, что у нас есть огромная страница, она гигантских размеров, а ещё у
нашего сайта просто немыслимая посещаемость. На этой странице сотни или даже тысячи `<a>`.
...
Для того чтобы это преодолеть — мы можем добавить более явный и специфический селектор,
...
И на самом деле мы можем сократить этот селектор ещё больше, чтобы он не был
перенасыщенным, вплоть до `.social-link{}`. Об этом читайте уже в следующем разделе…

Пожалуй, повторюсь: ваш ключевой селектор — это то, что определяет сколько работы
будет у браузера, так что *обращайте на него внимание*.

## Перенасыщенные селекторы

Итак, теперь мы знаем, что такое ключевой селектор, и что отсюда идёт большая
часть работы браузера. Теперь мы можем продолжать оптимизировать. Самое лучшее в
...
вплоть до элемента `html`.  Из-за этого браузер вынужден делать слишком много проверок,
которые по сути не нужны. Зная это, мы можем привести более реалистичные
примеры, вроде этого:
...
он применён, абсолютно неважен: значит, мы можем выбросить `ul`.
...
## Насколько всё это необходимо?

Короткий ответ: _не очень_.

Ответ поосновательней: _зависит от того, что вы разрабатываете_. Если вы работаете над
...
имеют значение, то — может быть да, а может быть и нет.
...
...пока всё остается по-прежнему, браузерам всё равно приходится делать ту работу,
о которой мы говорили выше, какими бы быстрыми они не становились. Даже если вам не нужно
(или вы просто не хотите) применять что-либо из описанного здесь, в любом случае
это определённо следует знать. Помните, селекторы могут быть дорогими в плане
...
Скорее всего вы делаете что-то неверно.

Я сам всё ещё относительный новичок в вопросах производительности CSS-селекторов,
так что если я что-то пропустил или вам есть что добавить, пожалуйста, пишите в
комментариях!

## Ещё об эффективности CSS-селекторов с точки зрения производительности
...
реально пригодится. Парень действительно шарит в этой теме!
